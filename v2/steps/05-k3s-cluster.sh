...
# Install k3s server
# --------------------------------------------------
lxd_exec "$SERVER_NODE" "systemctl is-active --quiet k3s || (curl -sfL https://get.k3s.io | sh -s - server --disable traefik)"

# Join token + server IP
SERVER_IP="$(sudo lxc list "$SERVER_NODE" -c 4 --format csv | awk '{print $1}' | cut -d' ' -f1)"
TOKEN="$(sudo lxc exec "$SERVER_NODE" -- bash -lc "cat /var/lib/rancher/k3s/server/node-token" 2>>"$LOG_FILE" | tr -d '\r')"

# Kubeconfig to host (root)
...
for n in "${AGENT_NODES[@]}"; do
  lxd_exec "$n" "systemctl is-active --quiet k3s-agent || (K3S_URL=https://${SERVER_IP}:6443 K3S_TOKEN=${TOKEN} curl -sfL https://get.k3s.io | sh -s - agent --node-name ${n})"
done
